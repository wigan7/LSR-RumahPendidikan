    ...
    function updateSpaceTravel(dt) {
      ...
      if(Math.random() < (0.03 + (currentPlanetIndex*0.005)) * 60 * dt) { let maxHP = Math.min(5, 2+Math.floor(currentPlanetIndex/1.5)); let hp = Math.random() < 0.2+(currentPlanetIndex*0.1) ? Math.floor(Math.random()*maxHP)+1 : 1; let size = 30 + (hp*12); let speed = (120 + Math.random() * 120); if (hp > 2) speed *= 0.6; asteroids.push({x:Math.random()*(canvas.width-size), y:-size, w:size, h:size, s:speed, r:0, rs:(Math.random()-0.5)*6*dt, hp:hp, maxHp:hp, hitTimer:0, grazed:false, img: Math.random()>0.5 ? 'asteroid1':'asteroid2'}); }
    ...
    function checkAnswer() {
      ...
      if(selectedOption === q.correct) {
        ...
        battleMessage = "SALAH! KENA TEMBAK!"; playerHP -= 20; battleMistakes++; SoundManager.play('wrong'); SoundManager.play('hit');
